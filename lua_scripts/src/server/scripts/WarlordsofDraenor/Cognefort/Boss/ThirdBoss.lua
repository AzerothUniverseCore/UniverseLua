...
function Therer.OnLeaveCombat(event, creature)
    creature:SendUnitYell("Préparez-vous à affronter mon pouvoir !", 0)
    creature:CastSpell(creature, 36393, true)
    creature:RemoveEvents()
end

function Therer.OnDied(event, creature, killer)
    if (killer:GetObjectType() == "Joueur") then
        killer:SendBroadcastMessage("Vous avez tué " .. creature:GetName() .. "!")
    end
    creature:RemoveEvents()
end
