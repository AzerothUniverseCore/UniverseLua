...
local DEFAULT_GLOBAL_COOLDOWN = 1.5
...
-- Safe wrapper around GetSpellInfo to avoid "Invalid spell slot" errors
-- that occur when called during addon load before the spell cache is ready.
local function safeGetSpellInfo(spellId)
    if not spellId then
        return nil
    end
    local ok, name, rank, icon, cost, isFunnel, powerType, castTime, minRange, maxRange = pcall(GetSpellInfo, spellId)
    if not ok then
        return nil
    end
...
-- Safe wrapper around GetSpellLink to avoid "Invalid spell slot" errors.
local function safeGetSpellLink(bookIndex, bookType)
    if not bookIndex then
        return nil
    end
    local ok, link = pcall(GetSpellLink, bookIndex, bookType)
    if not ok then
        return nil
    end
    return link
...
local function addSpellInfoFromBook(ids, names, bookIndex, bookType)
    ...
    local spellName = safeGetSpellInfo(bookIndex, bookType or BOOKTYPE_SPELL)
    addSpellName(names, spellName)

    local link = safeGetSpellLink(bookIndex, bookType or BOOKTYPE_SPELL)
...
local function isActiveGlobalCooldown(start, duration, enable)
    if not start or not duration or enable == 0 or start <= 0 or duration <= 0 then
        return false
    end

    return duration <= 2.5 and (start + duration) > GetTime()
end

local function startManualGlobalCooldown()
    local start, duration, enable = nil, nil, nil
    if GetSpellCooldown then
        start, duration, enable = GetSpellCooldown(GLOBAL_COOLDOWN_SPELL_ID)
    end

    if not isActiveGlobalCooldown(start, duration, enable) then
        start = GetTime()
        duration = DEFAULT_GLOBAL_COOLDOWN
        enable = 1
    end

    SC.gcdStart = start
    SC.gcdDuration = duration
    SC.gcdEnable = enable
end

local function getGlobalCooldown(action)
    if action and SC.origGetActionCooldown then
        local start, duration, enable = SC.origGetActionCooldown(action)
        if isActiveGlobalCooldown(start, duration, enable) then
    ...
    local start, duration, enable = GetSpellCooldown(GLOBAL_COOLDOWN_SPELL_ID)
    if isActiveGlobalCooldown(start, duration, enable) then
        return start, duration, enable
    end

    if isActiveGlobalCooldown(SC.gcdStart, SC.gcdDuration, SC.gcdEnable) then
        return SC.gcdStart, SC.gcdDuration, SC.gcdEnable
    end
...
local function updateButton(button, data)
    ...
    local text = getChargeText(button)
    local remaining = 0
    local start, duration, enable = getGlobalCooldown(getButtonAction(button))
    ...
    if not start and data.currentCharges < data.maxCharges and remaining > 0 and data.effectiveCooldown > 0 then
        duration = data.effectiveCooldown / 1000
        start = data.rechargeStart or (GetTime() - (duration - remaining))
        enable = 1
    end
    ...
    if text and data.maxCharges > 2 then
...
GetActionCooldown = function(action)
    local data = findSpellDataForAction(action)
    if data then
        local start, duration, enable = getGlobalCooldown(action)
        if start then
            return start, duration, enable
        end
...
local function handleStatusRow(message)
    ...
    target[spellId] = {
        spellId = spellId,
        spellName = safeGetSpellInfo(spellId),
...
SC.onClientEvent = function(event, ...)
    if event == "PLAYER_LOGIN" or event == "PLAYER_ENTERING_WORLD" then
        SC.loginDelay = 2
    elseif event == "UNIT_SPELLCAST_START" then
        local unit, spellName = ...
        if unit == "player" and spellName then
            SC.startedCastSpells = SC.startedCastSpells or {}
            SC.startedCastSpells[spellName] = GetTime()
        end
    elseif event == "UNIT_SPELLCAST_SUCCEEDED" then
        local unit, spellName = ...
        local startedAt = spellName and SC.startedCastSpells and SC.startedCastSpells[spellName]
        if unit == "player" and (not startedAt or (GetTime() - startedAt) > 0.25) then
            startManualGlobalCooldown()
        end
    end
...
SC.onClientUpdate = function(elapsed)
...
SC.frame:RegisterEvent("UNIT_SPELLCAST_START")
SC.frame:RegisterEvent("UNIT_SPELLCAST_SUCCEEDED")

SC.frame:SetScript("OnEvent", function(_, event, ...)
    SC.onClientEvent(event, ...)
end)

SC.frame:SetScript("OnUpdate", function(_, elapsed)
    SC.onClientUpdate(elapsed)
end)
