local NPC_SOLDAT_COLE = 160664
local QUEST_DISCIPLE_CHALLENGE = 58209
    ...
    if creature:GetFaction() == FACTION_HOSTILE then
        ...
        if healthAfterDamage <= thresholdHealth then
            ...
            if attacker and attacker:ToPlayer() then
                ...
                player:SendBroadcastMessage("Victoire ! Soldat Cole se rend.")
...
-- Enregistrer les gossips pour Soldat Cole
RegisterCreatureGossipEvent(NPC_SOLDAT_COLE, 1, OnGossipHello)
RegisterCreatureGossipEvent(NPC_SOLDAT_COLE, 2, OnGossipSelect)

-- Enregistrer les événements de combat
RegisterCreatureEvent(NPC_SOLDAT_COLE, 2, OnLeaveCombat)
RegisterCreatureEvent(NPC_SOLDAT_COLE, 4, OnDied)
RegisterCreatureEvent(NPC_SOLDAT_COLE, 5, OnSpawn)
RegisterCreatureEvent(NPC_SOLDAT_COLE, 9, OnDamageTaken)