...
local CommonMountList =	{	17481,	-- Rivendare's Deathcharger
...
local HordeMountList 	= {	578,	-- Black Wolf						#Horde
...
local function GetSpell(event, player)
	for c, mountSpell in ipairs(CommonMountList) do
		if (player:HasSpell(mountSpell) == true) then
			local getAccountSpell = AuthDBQuery('SELECT commonMountID FROM auc_eluna.mod_link_account_mount WHERE accountID = '..player:GetAccountId()..' AND commonMountID = '..mountSpell..';');
			if getAccountSpell == nil then
				local SetAccountSpell = AuthDBQuery('INSERT IGNORE INTO auc_eluna.mod_link_account_mount VALUES ('..player:GetAccountId()..', '..mountSpell..', '..EMPTY_VALUE..', '..EMPTY_VALUE..');');
			end
		end	
	end	
	if (player:IsAlliance() == true) then
		for c, mountSpell in ipairs(AllianceMountList) do
			if (player:HasSpell(mountSpell) == true) then
				local getAccountSpell = AuthDBQuery('SELECT allianceMountID FROM auc_eluna.mod_link_account_mount WHERE accountID = '..player:GetAccountId()..' AND allianceMountID = '..mountSpell..';');
				if getAccountSpell == nil then
					local SetAccountSpell = AuthDBQuery('INSERT IGNORE INTO auc_eluna.mod_link_account_mount VALUES ('..player:GetAccountId()..', '..EMPTY_VALUE..', '..mountSpell..', '..EMPTY_VALUE..');');
				end
			end	
		end	
	elseif (player:IsHorde() == true) then
		for c, mountSpell in ipairs(HordeMountList) do
			if (player:HasSpell(mountSpell) == true) then
				local getAccountSpell = AuthDBQuery('SELECT hordeMountID FROM auc_eluna.mod_link_account_mount WHERE accountID = '..player:GetAccountId()..' AND hordeMountID = '..mountSpell..';');
				if getAccountSpell == nil then
					local SetAccountSpell = AuthDBQuery('INSERT IGNORE INTO auc_eluna.mod_link_account_mount VALUES ('..player:GetAccountId()..', '..EMPTY_VALUE..', '..EMPTY_VALUE..', '..mountSpell..');');
				end
			end
		end 
	end
...
local function SetSpell (event, player)
	if (player:GetLevel() >= 20) then
		local GetSpell = AuthDBQuery('SELECT commonMountID FROM auc_eluna.mod_link_account_mount WHERE accountid = '..player:GetAccountId()..';');
		...
		if (player:IsAlliance() == true) then
			local GetSpell = AuthDBQuery('SELECT allianceMountID FROM auc_eluna.mod_link_account_mount WHERE accountid = '..player:GetAccountId()..';');
			if GetSpell ~= nil then
				repeat
				local mountID = GetSpell:GetUInt32(0);
				if (mountID ~= EMPTY_VALUE and player:HasSpell(mountID) == false) then
		...
		elseif (player:IsHorde() == true) then
			local GetSpell = AuthDBQuery('SELECT hordeMountID FROM auc_eluna.mod_link_account_mount WHERE accountid = '..player:GetAccountId()..';');
			if GetSpell ~= nil then
				repeat
				local mountID = GetSpell:GetUInt32(0);
				if (mountID ~= EMPTY_VALUE and player:HasSpell(mountID) == false) then
