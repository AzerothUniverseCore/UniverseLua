...
CharDBQuery([[CREATE TABLE IF NOT EXISTS `]] .. Rebirth.Config.DbName .. [[`.`Rebirth_characters` (
    ...
    if(Rebirth.Level[pGuid].level == 0 ) then
	  ...
	  if (player:GetCoinage() >= 0) then
        ...
        player:SetLevel(80)
            ...
            player:SendNotification('|cff00f0f0Vous avez commencé votre rebirth, le sésame ne fonctionnera pas. Vous avez été remboursé de 420 Breloques Supérieur.|r')

            -- Remboursement de la monnaie 7000655 x420
            player:AddItem(7000655, 420) 
